const fs = require("fs");
const data = JSON.parse(fs.readFileSync("data/osm-ngos.json", "utf-8"));
...
console.log(`Total: ${data.length} NGOs (office=ngo only)\n`);
...
const withRealDesc = data.filter((p) => p.description && !p.description.includes("is a ngo")).length;
...
fs.writeFileSync("data/ngo-sample.txt", lines);
console.log("\n--- Sample of 100 ---\n");
console.log(lines);
